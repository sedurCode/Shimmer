function soundsasleep1(comPort, PPGChannelNum, captureDuration, fileName)
...
%% PPG2HR settings
numberOfBeatsToAve = 10;                                                    % the number of consecutive heart beats that are averaged to calculate the heart rate. Instantaneous heart rate is calculated after each detected pulse. So the last X instantaneous heart rates are averaged to give the output (where X is numberOfBeatsToAve)) must be >= 1.
useLastEstimate = 0;                                                       % true for repeating last valid estimate when invalid data is detected.
...
elseif (shimmer.connect)                                                       % TRUE if the shimmer connects
    ...
    shimmer.setenabledsensors(SensorMacros.ACCEL,1,SensorMacros.MAG,1,...  % Enable the shimmer accelerometer, magnetometer, gyroscope and battery voltage monitor
        SensorMacros.GYRO,1, SensorMacros.INTA13, 1,SensorMacros.INTA14, 1,SensorMacros.GSR, 1);
    ...
    if (shimmer.start)                                                     % TRUE if the shimmer starts streaming
        ...
        set(h.figure1, 'WindowStyle', 'docked');
        startTime = now();
        ...
        while (elapsedTime < captureDuration)
            ...
            if (firsttime==true && isempty(newData)~=1)
                ...
                firsttime = writeHeadersToFile(fileName,...
                    [signalNameArray {char('Heart Rate'), char('PPG Filtered')}],...
                    [signalFormatArray {char('cal'), char('tab')}],...
                    [signalUnitArray {char('mV'), char('BPM')}]);
            end
            ...
            if ~isempty(newData)                                                            % TRUE if new data has arrived
                ...
                % catch time stamp and modify
                newData(:,chIndex(1)) = seconds( startTime + milliseconds(newData(:,chIndex(1))));
                % To decode timestamp, you must convert to seconds and then datestr 
